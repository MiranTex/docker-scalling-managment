...
// arranque do serviço. Mesmo padrão de services/templatesadmin/internal/store/migrate.go.
func Migrate(ctx context.Context, db *sql.DB) error {
	if _, err := db.ExecContext(ctx, `CREATE SCHEMA IF NOT EXISTS launcher`); err != nil {
		return fmt.Errorf("store: criando schema launcher: %w", err)
	}
...
func isApplied(ctx context.Context, db *sql.DB, version string) (bool, error) {
	var exists bool
	err := db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM launcher.schema_migrations WHERE version = $1)`, version).Scan(&exists)
	if err != nil {
...
func applyMigration(ctx context.Context, db *sql.DB, version string) error {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO launcher.schema_migrations (version) VALUES ($1)`, version); err != nil {
