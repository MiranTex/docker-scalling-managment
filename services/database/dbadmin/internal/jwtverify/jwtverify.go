...
//
// A lógica de verificação (parse de header/claims, checagem de
// alg/assinatura/iss/aud/exp) espelha deliberadamente
// services/auth/internal/token.Manager.Verify -- não dá para importar
// aquele pacote diretamente (vive em auth/internal/token, e o Go proíbe
// importar um pacote internal de fora do módulo que o contém), e este
// serviço só tem a chave pública mesmo (via JWKS), nunca a privada, então
// nem faria sentido construir um token.Manager (que assume posse da
// privada).
package jwtverify
