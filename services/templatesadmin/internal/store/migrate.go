...
// arranque do serviço. Mesmo padrão de services/secretsadmin/internal/store/migrate.go.
func Migrate(ctx context.Context, db *sql.DB) error {
	if _, err := db.ExecContext(ctx, `CREATE SCHEMA IF NOT EXISTS service_templates`); err != nil {
		return fmt.Errorf("store: criando schema service_templates: %w", err)
	}
	if _, err := db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS service_templates.schema_migrations (
...
func isApplied(ctx context.Context, db *sql.DB, version string) (bool, error) {
	var exists bool
	err := db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM service_templates.schema_migrations WHERE version = $1)`, version).Scan(&exists)
	if err != nil {
...
func applyMigration(ctx context.Context, db *sql.DB, version string) error {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO service_templates.schema_migrations (version) VALUES ($1)`, version); err != nil {
